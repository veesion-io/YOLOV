...
vid_classes = (
    ...
    "Shopping Basket",
    "Grocery Bag",
    "Fruit and Vegetable Bag (plastic or paper)",
    "Sports Bag",
    "Cooler Bag",
    "Shop's Grocery Cart",
    "Travelling Bag",
...
class VIDEvaluator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vid_to_coco = {
            ...
            "annotations": [],
            "categories": [
                {"supercategorie": "", "id": 0, "name": "Backpack"},
                {"supercategorie": "", "id": 1, "name": "Handbag"},
                {"supercategorie": "", "id": 2, "name": "Tote Bag"},
                {"supercategorie": "", "id": 3, "name": "Banana Bag / Satchel"},
                {"supercategorie": "", "id": 4, "name": "Shopping Cart Bag"},
                {"supercategorie": "", "id": 5, "name": "Shopping Basket"},
                {"supercategorie": "", "id": 6, "name": "Grocery Bag"},
                {
                    "supercategorie": "",
                    "id": 7,
                    "name": "Fruit and Vegetable Bag (plastic or paper)",
                },
                {"supercategorie": "", "id": 8, "name": "Sports Bag"},
                {"supercategorie": "", "id": 9, "name": "Cooler Bag"},
                {"supercategorie": "", "id": 10, "name": "Shop's Grocery Cart"},
                {"supercategorie": "", "id": 11, "name": "Travelling Bag"},
                {"supercategorie": "", "id": 12, "name": "Shop's trolley"},
            ],
            "images": [],
            "licenses": [],
        }
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, _, info_imgs, label, path, time_embedding) in enumerate(
            ...
            with torch.no_grad():
                ...
                if is_time_record:
                    start = time.time()
                outputs, ori_res = model(imgs, lframe=self.lframe, gframe=self.gframe)
            ...
            if self.kwargs.get("first_only", False):
                ...
                label = [label[0]]
            temp_data_list, temp_label_list = self.convert_to_coco_format(
                outputs, info_imgs, copy.deepcopy(label)
            )
    ...
    def convert_to_coco_format(self, outputs, info_imgs, labels):
        ...
        for output, info_img, _label in zip(outputs, info_imgs, labels):
            # if frame_now>=self.lframe: break
            scale = min(
                self.img_size[0] / float(info_img[0]),
                self.img_size[1] / float(info_img[1]),
            )
    ...
    def evaluate_prediction(self, data_dict, statistics, ori=False):
        ...
        a_infer_time = (
            1000
            * inference_time
            / (n_samples * self.dataloader.batch_sampler.batch_size)
        )
        a_nms_time = (
            1000 * nms_time / (n_samples * self.dataloader.batch_sampler.batch_size)
        )
        time_info = ", ".join(
            [
