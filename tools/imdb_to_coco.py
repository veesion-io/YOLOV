...
from yolox.utils import is_main_process, xyxy2xywh
...
def make_parser():
    ...
    parser.add_argument(
        "--dataset_dir",
        default="/mnt/weka/scratch/yuheng.shi/dataset/VID",
...
class REPP_to_COCO_Evaluator:
    ...
    def __init__(self, pred_coco, num_classes, dataset_dir, testdev=False):
        """
        Args:
        ...
        self.vid_to_coco = {
            ...
            "annotations": [],
            "categories": [
                {"supercategorie": "", "id": 0, "name": "airplane"},
    ...
    def evaluate(
        ...
        progress_bar = tqdm if is_main_process() else iter
        vid_num = 0
        for vid, video_preds in progress_bar(
            get_video_frame_iterator(self.pred_coco)
        ):  # video level
...
if __name__ == "__main__":
    ...
    if args.tide:
        ...
        tide = TIDE()
        bbox_results = datasets.COCOResult(
            args.path.replace(".pkl", "_imdb2coco.json")
        )  # These files were downloaded above.
